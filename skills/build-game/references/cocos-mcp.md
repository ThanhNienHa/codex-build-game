...
7. Start only one fresh preview, check startup errors and the changed gameplay path, then stop it when validation is complete unless the user asks to leave it running. For significant work, record the lifecycle and observed path using `references/evidence.md`.
